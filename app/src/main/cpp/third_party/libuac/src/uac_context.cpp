...
namespace uac {
    ...
    uac_context_impl::uac_context_impl(libusb_context *libusb_ctx)
            : libusb_ctx(libusb_ctx), owns_libusb_ctx(libusb_ctx == nullptr), alive(true) {
        LOG_DEBUG("create context with usb context: %p", libusb_ctx);
        if (owns_libusb_ctx) {
            // Use a private context. The default context is global and can be
            // torn down by another UAC instance while this event thread runs.
            // Skip device discovery: Android only grants this process access
            // to the already-authorized UsbDeviceConnection fd.
            libusb_init_option option{};
            option.option = LIBUSB_OPTION_NO_DEVICE_DISCOVERY;
            int errval = libusb_init_context(&this->libusb_ctx, &option, 1);
            if (errval != LIBUSB_SUCCESS || this->libusb_ctx == nullptr) {
                this->libusb_ctx = nullptr;
                alive = false;

...
JNIEXPORT jboolean JNICALL Java_com_llawsxx_audioprocess_NativeAudio_start(JNIEnv*e,jobject o,jint rate,jint frames,jint inDev,jint outDev,jint channels,jint pair,jboolean useNetworkInput,jint usbFd,jboolean usbInputHost,jboolean usbOutputHost,jint usbInputBurstPackets,jint usbOutputBurstPackets){
    ...
    if(usbFd >= 0 && (g.usb_input_host || g.usb_output_host)) {
        g.usb_audio = usb_host_audio_start(usbFd, rate, g.usb_input_host, g.usb_output_host,
...
JNIEXPORT jlongArray JNICALL Java_com_llawsxx_audioprocess_NativeAudio_usbStats(JNIEnv*e,jobject o){(void)o;usb_host_audio_stats_t s={0};usb_host_audio_get_stats(g.usb_audio,&s);jlong values[6]={(jlong)s.input_packet_errors,(jlong)s.input_empty_packets,(jlong)s.input_transfer_errors,(jlong)s.input_ring_overruns,(jlong)s.output_transfer_errors,(jlong)s.output_low_water_events};jlongArray result=(*e)->NewLongArray(e,6);(*e)->SetLongArrayRegion(e,result,0,6,values);return result;}
